...
const main = async (event, context) => {
  ...
  try {
    ...
    //MTH Accept all
    let policyStmnts = tmpPolicyStatements(methodArnSegments)
    ...
    //   'custom:acl': jwtDecoded['custom:acl']
    // }
  ...
  //   }
  // } catch (error) {
  //   console.log('Error Generating Allow response: ', JSON.stringify(error, null, 2))
  // }

  return denyAllResponse(principalId)
}
...
const tmpPolicyStatements = function (methodArnSegments) {
  let statements = []
...
const denyAllResponse = function (principalId) {
  return {
    "principalId": principalId,
    "policyDocument": {
      "Version": "2012-10-17",
